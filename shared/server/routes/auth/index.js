...
function createNewUser(user, cb){
  mongoose.model('User').create(user, function (err, userObject) {
    ...
    mongoose.model('Group').collection.insert(arrGroups, function(err, data){
      console.log('default groups added');
    });
    cb(err,userObject);
  });
}
function removeUser(user_id, cb){
  mongoose.model('GroupContact').remove({user_id: user_id}, function(err, ress){});
  mongoose.model('Group').remove({user_id: user_id}, function(err, data){});
  mongoose.model('Contact').remove({user_id: user_id}, function(err, data){});
  mongoose.model('ActivityLog').remove({user_id: user_id}, function(err, data){});
  mongoose.model('Task').remove({user_id: user_id}, function(err, data){});
  mongoose.model('MergeHistory').remove({user_id: user_id}, function(err, data){});
  ...
  .post(function(req, res, next) {
    ...
    var promise = new Promise(function(resolve, reject){
      if(!req.body.username){
        ...
        config.nas_protocol = req.protocol;
    ...
    promise.then(function(result){
      ...
      if(user){
        ...
        token_auth.is_nas_user = user.is_nas_user || false;
        token_auth.is_tutorial_displayed = user.is_tutorial_displayed || false;
      }
      var response = {};
